...
export const restAssert = {
  expectSuccess(response: Response, expectedStatus = 200) {
    assert.equal(response.status, expectedStatus, `Expected status ${String(expectedStatus)}, got ${String(response.status)}`);
  ...
  expectError(response: Response, expectedStatus: number, expectedMessage?: string) {
    ...
    if (expectedMessage) {
      assert.ok(body.error.includes(expectedMessage), `Expected error to contain "${expectedMessage}", got "${body.error}"`);
    }
  },
};
